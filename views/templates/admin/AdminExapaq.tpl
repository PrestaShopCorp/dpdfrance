	...
	<input id="tableFilter" value="{l s='Search something, separate values with ; ' mod='exapaq'}"/><img id="filtericon" src="../modules/exapaq/views/img/admin/search.png"/><br/><br/>
		...
		<body><table>
				<thead>
					<tr>
						...
						<th class="hid">ID</th>
						<th class="href">{l s='Reference' mod='exapaq'}</th>
						<th class="hdate">{l s='Date of order' mod='exapaq'}</th>
						<th class="hnom">{l s='Recipient' mod='exapaq'}</th>
						<th class="htype">{l s='Service' mod='exapaq'}</th>
						<th class="hpr">{l s='Destination' mod='exapaq'}</th>
						<th class="hpoids">{l s='Weight' mod='exapaq'}</th>
						<th colspan="2" class="hprix" align="right">{l s='Amount' mod='exapaq'}<br/><span style="font-size:10px;">{l s='(tick to insure<br/>this parcel)' mod='exapaq'}</span></th>
						<th class="hstatutcommande" align="center">{l s='Order status' mod='exapaq'}</th>
						<th class="hstatutcolis" align="center">{l s='Parcel trace' mod='exapaq'}</th>
					</tr>
		...
		{foreach from=$order_info item=order}
			<tr>
				...
				<td class="statutcolis" align="center"><a href="javascript:void(0)" onclick="window.open('http://webtrace.exapaq.com/exa-webtrace/webclients.aspx?verknr={$order.reference|escape:'htmlall':'UTF-8'}&amp;kund_mandnr={$order.depot_code|escape:'htmlall':'UTF-8'}&amp;kundenr={$order.shipper_code|escape:'htmlall':'UTF-8'}&amp;cmd=VERKNR_SEARCH','','width=1024,height=768,top=30,left=20')">{$order.dernier_statut_colis|escape:'quotes':'UTF-8'}</a></td>
			</tr>
		{/foreach}
	</tbody></table>
	<p>
		<input type="submit" class="button" name="exportOrders" value="{l s='Export selected orders to EXA-Print' mod='exapaq'}" />
		<input type="submit" class="button" name="updateShippedOrders" value="{l s='Update shipped orders' mod='exapaq'}" />
		<input type="submit" class="button" name="updateDeliveredOrders" value="{l s='Update delivered orders' mod='exapaq'}" />
	</p>
	</form></fieldset>
{else}
	<div class="alert warn">{l s='There are no orders' mod='exapaq'}</div>
{/if}